...
class EventEmitter implements RawEventEmitter {
    ...
    public on(eventName: string | symbol, listener: (...args: unknown[]) => void): this {
        if (this._listeners.size >= this._maxListeners) {
            console.warn("Max listeners reached, possible memory leak or whatever this warning normally says, increase via <EventEmitter>.setMaxListeners");
        }
    ...
    public off(eventName: string | symbol, listener: (...args: unknown[]) => void): this {
        ...
        this._listeners.set(eventName, listeners.filter((l) => l !== listener));
