...
class PermissionHandler {
  ...
  public constructor(
    ...
    this.channels =
      channels?.map((channel) => ({
        id: channel.id,
        overrides: Object.entries(channel.permissionOverrides).map(
          ([id, override]) => ({
            allow: new Permissions(override.allow),
            deny: new Permissions(override.deny),
            id,
            type:
              override.type === permissionOverrideTypes.Member
                ? "Member"
                : "Role",
          }),
        ),
  ...
  public canManageRole(role: {
    ...
    const membersHighestRole = this.memberRoles.sort(
  ...
  public hasChannelPermission(
    ...
    const overrides = channel.overrides.filter(
      (Override) =>
        Override.id === this.guildMemberId ||
        this.memberRoles.some((Role) => Role.id === Override.id),
    );
