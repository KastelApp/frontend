...
export const useRoleStore = create<RoleStore>((set, get) => ({
  ...
  addRoles: (roles: Role[]) =>
    set(() => ({ roles: [...get().roles, ...roles] })),
  removeRole: (role: Role) =>
    set((state) => ({ roles: state.roles.filter((r) => r.id !== role.id) })),
  getCurrentRoles: () => {
...
export const useInviteStore = create<InviteStore>((set) => ({
  ...
  removeInvite: (invite: string) =>
    set((state) => ({ invites: state.invites.filter((i) => i !== invite) })),
  version: 0,
  setVersion: (version: number) => set(() => ({ version })),
}));
...
export const useMemberStore = create<MemberStore>((set, get) => ({
  ...
  removeMember: (member: Member) =>
    set((state) => ({
      members: state.members.filter((m) => m.userId !== member.userId),
    })),
  ...
  getCurrentMember: () => {
    ...
    return (
      get().members.find(
        (m) => m.userId === user.id && m.guildId === match[1],
      ) ?? null
    );
  },
  getCurrentMembers: () => {
...
export const useMessageStore = create<MessageStore>((set, get) => ({
  ...
  getAuthor: (messageId: string) => {
    ...
    return (
      useUserStore.getState().users.find((u) => u.id === message.authorId) ??
      null
    );
  },
  getMember: (messageId: string) => {
    ...
    return (
      useMemberStore
        .getState()
        .members.find((m) => m.userId === message.authorId) ?? null
    );
  },
  getCurrentMessages: () => {
    if (!("window" in globalThis)) return [];
...
export const useChannelStore = create<ChannelStore>((set, get) => ({
  ...
  addChannel: (channel: BaseChannel) =>
    set((state) => ({ channels: [...state.channels, channel] })),
  addChannels: (channels: BaseChannel[]) =>
    set((state) => ({ channels: [...state.channels, ...channels] })),
  removeChannel: (channel: BaseChannel) =>
    set((state) => ({
      channels: state.channels.filter((c) => c.id !== channel.id),
    })),
  getCurrentChannel: () => {
    if (!("window" in globalThis)) return null;
...
export const useChannelStoreWithNoMessages =
  create<ChannelStoresWithNoMessages>((set) => ({
    channels: [],
    ...
    return set(() => {
      ...
      if (typing.some((t) => t.userId === userId)) {
        return {
          typing: {
            ...get().typing,
            [channelId]: typing.map((t) =>
              t.userId === userId ? { userId, since } : t,
            ),
          },
        };
      }
