  ...
  const readyData: {
    ...
    users: [
      new User(
        ...
        true,
        false,
        data.settings as unknown as SettingsPayload,
      ),
    ],
    guilds: [],
  };
  ...
  for (const guild of data.guilds) {
    ...
    for (const channel of guild.channels) {
      switch (channel.type) {
        ...
        case constants.channelTypes.GuildMarkdown: {
          readyData.channels.push(
            new MarkdownBasedChannel(ws, channel, guild.id),
          );
    ...
    for (const member of guild.members) {
      if (!readyData.users.find((o) => o.id === member.user.id)) {
        readyData.users.push(
          new User(ws, member.user, member.presence, false, true),
        );
      } else if (
        readyData.users.find((o) => o.id === member.user.id && o.partial)
      ) {
        readyData.users = [
      ...
      if (
        !readyData.members.find(
          (o) => o.userId === member.user.id && o.guildId === guild.id,
        )
      ) {
        ...
        readyData.members.find(
          (o) =>
            o.userId === member.user.id && o.guildId === guild.id && o.partial,
        )
      ) {
        readyData.members = [
          ...readyData.members.filter(
            (o) => o.userId !== member.user.id && o.guildId !== guild.id,
          ),
    ...
    for (const coOwner of guild.coOwners) {
      if (!readyData.users.find((o) => o.id === coOwner.id)) {
        ...
        readyData.users.find((o) => o.id === coOwner.id && o.partial)
      ) {
        readyData.users = [
      ...
      const member = new Member(
        ws,
        { user: coOwner, owner: false, roles: [], nickname: null },
        guild.id,
        true,
      );
      ...
      if (
        !readyData.members.find(
          (o) => o.userId === coOwner.id && o.guildId === guild.id,
        )
      ) {
        readyData.members.push(member);
      } else if (
        readyData.members.find(
          (o) => o.userId === coOwner.id && o.guildId === guild.id && o.partial,
        )
      ) {
        readyData.members = [
          ...readyData.members.filter(
