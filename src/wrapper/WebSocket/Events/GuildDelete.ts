...
const guildDelete = (ws: Websocket, data: unknown) => {
  ...
  guildState.setGuilds(
    guildState.guilds.filter((guild) => guild.id !== data.guildId),
  );
  ...
  channelState.setChannels(
    channelState.channels.filter((channel) => {
  ...
  roleState.setRoles(
    roleState.roles.filter((role) => role.guildId !== data.guildId),
  );

  const memberState = useMemberStore.getState();

  memberState.setMembers(
    memberState.members.filter((member) => member.guildId !== data.guildId),
  );
  ...
  messageState.setMessages(
    messageState.messages.filter(
      (message) => !channelIdsBye.includes(message.channelId ?? ""),
    ),
  );
};
