...
class Guild {
    ...
    public async leave() {
        if (this.partial) {
            StringFormatter.log(
                `${StringFormatter.purple("[Wrapper]")} ${StringFormatter.green("[Guild]")} ${StringFormatter.white(`[${this.id} (${this.name})] Cannot leave a partial guild.`)}`
            )

            return;
        }

        const request = await this.#ws.client?.api.delete(`/guilds/${this.id}/leave`);

        if (!request?.ok || request.status !== 204) {
            StringFormatter.log(
                `${StringFormatter.purple("[Wrapper]")} ${StringFormatter.green("[Guild]")} ${StringFormatter.white(`[${this.id} (${this.name})] Failed to leave guild.`)}`
            )
