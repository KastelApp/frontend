...
class Message {
    ...
    public constructor(ws: Websocket, data: Partial<MessageType>, state: "sent" | "deleted" | "sending" | "failed" = "sent", channelId?: string) {
        this.#ws = ws;
        ...
        if (data.replyingTo && typeof data.replyingTo === "object") {
            if ("content" in data.replyingTo) {
                useMessageStore.getState().addMessage(new Message(ws, data.replyingTo, "sent", channelId));
    ...
    public fastUpdate(data: Partial<MessageType>, state: "sent" | "deleted" | "sending" | "failed" = "sent") {
        this._content = data.content ?? this.content;

        this._editedDate = data.editedDate ? new Date(data.editedDate) : this.editedDate;
        ...
        if (data.replyingTo && typeof data.replyingTo === "object") {
            if ("content" in data.replyingTo) {
                useMessageStore.getState().addMessage(new Message(this.ws, data.replyingTo, "sent", this.channelId));
                this._replyingTo = data.replyingTo.id;
            } else {
                this._replyingTo = data.replyingTo.messageId
            }
