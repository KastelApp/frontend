...
> {
  ...
  public has(bit: bigint | number | keyof (typeof this)["Flags"]) {
    const bits =
      typeof bit === "string" ? this.Flags[bit] ?? 0n : BigInt(bit as number);
  ...
  public toJSON(): Record<keyof (typeof this)["Flags"], boolean> {
    return Object.fromEntries(
      Object.entries(this.Flags).map(([key, value]) => [key, this.has(value)]),
  ...
  public clean(bits: (keyof (typeof this)["Flags"])[]) {
    ...
    for (const bit of bits) {
      if (this.has(bit)) finishedBits |= this.Flags[bit as never] ?? 0n;
    }
...
> {
  ...
  public clean(bits: (keyof (typeof this)["Flags"])[]) {
    ...
    for (const bit of bits) {
      if (this.has(bit)) finishedBits |= this.Flags[bit as never] ?? 0;
    }
