...
class Permissions {
  public bits: [
    bigint | string,
    FlagUtilsBInt<
      (typeof permissions)[keyof typeof permissions]["subPermissions"]
    >,
  ][];
    ...
    this.bits = bits.map(([group, subPermission]) => [
      ...
      >(
        ...
        Object.values(permissions).find(
          (permission) => permission.int === BigInt(group),
        )?.subPermissions ?? {},
      ),
    ]);
  }
  ...
  public has<T extends PermissionKey, HT extends hasType>(
    ...
    if (perms.includes("Administrator") && !ignoreAdmin) {
      if (
        this.bits.find(
          ([bits]) => BigInt(bits) === permissions.Administrator.int,
        )
      )
        return true;
    }
    ...
    for (const perm of perms) {
      ...
      const index = this.bits.findIndex(
        ([bits]) => BigInt(bits) === permissions[group].int,
      );
  ...
  public add<T extends PermissionKey>(perms: (T | "Administrator")[]): this {
    for (const perm of perms) {
      if (perm === "Administrator") {
        this.bits = [
          [
            ...
            new FlagUtilsBInt<
              (typeof permissions)[keyof typeof permissions]["subPermissions"]
            >(0n, permissions.Administrator.subPermissions),
          ],
        ];
      ...
      if (
        !this.bits.find(([perm]) => BigInt(perm) === permissions[group].int)
      ) {
        this.bits.push([
          permissions[group].int,
          new FlagUtilsBInt<
            (typeof permissions)[keyof typeof permissions]["subPermissions"]
          >(0n, permissions[group].subPermissions),
        ]);
      }
  ...
  private getGroupFromSubPermission(
    subPermission: PermissionKey,
  ): keyof typeof permissions | null {
    for (const group of Object.keys(permissions)) {
      if (
        subPermission in
        permissions[group as keyof typeof permissions].subPermissions
      )
  ...
  public remove<T extends PermissionKey>(perms: (T | "Administrator")[]): this {
    for (const perm of perms) {
      if (perm === "Administrator") {
        this.bits = this.bits.filter(
          (bits) => BigInt(bits[0]) !== permissions.Administrator.int,
        );
  ...
  public toJSON() {
    ...
    for (const [group, stuff] of Object.entries(permissions)) {
      for (const [subPermission] of Object.entries(stuff.subPermissions)) {
        ...
        obj[group][subPermission] = this.has(
          [subPermission as PermissionKey],
          true,
          "some",
        );
      }
  ...
  public get normizedBits(): [string, string][] {
    return this.bits.map(([bits, subPermission]) => [
      String(bits),
      String(subPermission.bits),
    ]);
  }
}
