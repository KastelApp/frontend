import { privateFlags, publicFlags } from "$/utils/constants.ts";
import FlagUtilsBInt from "./NewFlags.ts";

const publicPrivateFlags: (keyof typeof privateFlags)[] = [
  "System",
  "Ghost",
  "Spammer",
  "VerifiedBot",
...
class FlagFields {
  ...
  public constructor(
    PrivFlags: bigint | number | string,
    PubFlags: bigint | number | string,
  ) {
  ...
  public has(
    bit: bigint | number | keyof typeof privateFlags | keyof typeof publicFlags,
  ) {
    return (
      this.PrivateFlags.has(bit as keyof typeof privateFlags) ||
      this.PublicFlags.has(bit as keyof typeof publicFlags)
    );
  }
  ...
  public static fromJSON(data: {
    PrivateFlags: bigint | number | string;
    PublicFlags: bigint | number | string;
  }) {
