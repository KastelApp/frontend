...
} from "$/types/http/user/settings.ts";
...
class User<IsClient extends boolean = false> {
  ...
  public constructor(
    ...
    this._id = ws.snowflake.validate(data.id ?? "")
      ? data.id!
      : ws.snowflake.generate(); // ? just generate a fake id, this shouldn't happen but we don't want to fail to generate a default avatar

    this._email = (isClient ? data.email : null) as IsClient extends true
      ? string
      : null;

    this._emailVerified = (
      isClient ? data.emailVerified : false
    ) as IsClient extends true ? boolean : false;
    ...
    this._phoneNumber = (
      isClient ? data.phoneNumber : null
    ) as IsClient extends true ? string : null;

    this._mfaEnabled = (
      isClient ? data.mfaEnabled : false
    ) as IsClient extends true ? boolean : false;

    this._mfaVerified = (
      isClient ? data.mfaVerified : false
    ) as IsClient extends true ? boolean : false;
  ...
  public get currentPresence() {
    ...
    return this.presence.some((p) => p.status === "invisible")
      ? "invisible"
  ...
  public get customStatus() {
    return (
      this.presence.find((p) => p.status !== "offline" && p.state !== null)
        ?.state ?? null
    );
  }
  ...
  public async updateSettings(
    ...
    const request = await this.#ws.client?.api.patch<
      SettingsPayload,
      UpdateSettingsOptions
    >("/users/@me/settings", options);
