...
const fuzzInReverse = (unicode: string) => {
	const foundEmoji = emojiDatasource.find((emoji) => {
		...
		const nonQualified =
			emoji.non_qualified && unicode.toLowerCase().startsWith(fixUnicode(emoji.non_qualified).toLowerCase());
...
const fuzzyParse = (unicode: string) => {
	const foundEmoji = emojiDatasource.find((emoji) => {
		...
		const nonQualified =
			emoji.non_qualified && fixUnicode(emoji.non_qualified).toLowerCase().startsWith(unicode.toLowerCase());
...
const parse = (
	unicode: string,
	options: {
		style?: "twemoji" | "noto-emoji" | "fluentui-emoji";
		baseUrl?: string;
	},
) => {
...
const findEmoji = (query: string, mode: "name" | "unicode"): Emoji | null => {
	const emojiArray = Object.values(emojiData).flat();

	const searchEmoji = (emoji: Emoji): Emoji | null => {
		if (mode === "name" && emoji.names.includes(query)) {
			return emoji;
		}

		if (mode === "unicode" && emoji.unicodeEmoji === query) {
			return emoji;
		}

		if (emoji.hasTone && emoji.toneChildren) {
			for (const child of emoji.toneChildren) {
				const found = searchEmoji(child as unknown as Emoji);
