...
const fuzzInReverse = (unicode: string) => {
  const foundEmoji = emojiDatasource.find((emoji) => {
    const uni = unicode
      .toLowerCase()
      .startsWith(fixUnicode(emoji.unified).toLowerCase());
    ...
    const nonQualified =
      ...
      unicode
        .toLowerCase()
        .startsWith(fixUnicode(emoji.non_qualified).toLowerCase());
    ...
    const googleEmoji =
      emoji.google &&
      unicode.toLowerCase().startsWith(emoji.google.toLowerCase());
    ...
    if (emoji.skin_variations) {
      for (const skin of Object.values(emoji.skin_variations)) {
        ...
        if (
          ...
          unicode
            .toLowerCase()
            .startsWith(fixUnicode(skin.non_qualified).toLowerCase())
        ) {
          return true;
        }
        ...
        if (
          skin.google &&
          unicode.toLowerCase().startsWith(skin.google.toLowerCase())
        ) {
          return true;
        }
      }
  ...
  const skinTone =
    foundEmoji.skin_variations &&
    Object.entries(foundEmoji.skin_variations).find(
      ([, skin]) =>
        unicode.toLowerCase().startsWith(skin.unified.toLowerCase()) ||
        (skin.non_qualified &&
          unicode
            .toLowerCase()
            .startsWith(fixUnicode(skin.non_qualified).toLowerCase())) ||
  ...
  return {
    name: skinTone
      ? skinTone[1].image.slice(0, -4)
      : foundEmoji.image.slice(0, -4),
    skinTone: !!skinTone,
    skinToneType: skinTone
      ? skinTones.find(
          (skin) => skin.code.toLowerCase() === skinTone[0].toLowerCase(),
        )
      : null,
  };
};
...
const fuzzyParse = (unicode: string) => {
  const foundEmoji = emojiDatasource.find((emoji) => {
    ...
    const nonQualified =
      emoji.non_qualified &&
      fixUnicode(emoji.non_qualified)
        .toLowerCase()
        .startsWith(unicode.toLowerCase());
    ...
    const googleEmoji =
      emoji.google &&
      emoji.google.toLowerCase().startsWith(unicode.toLowerCase());
    ...
    if (emoji.skin_variations) {
      for (const skin of Object.values(emoji.skin_variations)) {
        ...
        if (
          skin.google &&
          skin.google.toLowerCase().startsWith(unicode.toLowerCase())
        ) {
          return true;
        }
      }
  ...
  const skinTone =
    ...
    Object.entries(foundEmoji.skin_variations).find(
      ([, skin]) =>
        skin.unified.toLowerCase().startsWith(unicode.toLowerCase()) ||
        (skin.non_qualified &&
          fixUnicode(skin.non_qualified)
            .toLowerCase()
            .startsWith(unicode.toLowerCase())) ||
...
const emojiParser = (unicode: string) => {
  const foundEmoji = emojiDatasource.find((emoji) => {
    const uni =
      fixUnicode(emoji.unified).toLowerCase() === unicode.toLowerCase();
    ...
    const nonQualified =
      emoji.non_qualified &&
      fixUnicode(emoji.non_qualified).toLowerCase() === unicode.toLowerCase();
    ...
    const googleEmoji =
      emoji.google && emoji.google.toLowerCase() === unicode.toLowerCase();
    ...
    if (emoji.skin_variations) {
      for (const skin of Object.values(emoji.skin_variations)) {
        ...
        if (
          skin.non_qualified &&
          fixUnicode(skin.non_qualified).toLowerCase() === unicode.toLowerCase()
        ) {
          return true;
        }
        ...
        if (
          skin.google &&
          skin.google.toLowerCase() === unicode.toLowerCase()
        ) {
          return true;
        }
      }
  ...
  const skinTone =
    ...
    Object.entries(foundEmoji.skin_variations).find(
      ([, skin]) =>
        skin.unified.toLowerCase() === unicode.toLowerCase() ||
        (skin.non_qualified &&
          fixUnicode(skin.non_qualified).toLowerCase() ===
            unicode.toLowerCase()) ||
...
const emojiToUnicode = (emoji: string) => {
  ...
  return scalars
    .map((scalar) => scalar?.codePointAt(0)?.toString(16))
    .join("-");
};
...
const parse = (
  unicode: string,
  options: {
    style: "twemoji" | "fluentui-emoji" | "noto-emoji";
    baseUrl?: string;
  },
) => {
