...
const ResizablePanel = forwardRef<ResizablePanelHandle, ResizablePanelProps>((props, ref) => {
	...
	return (
		<div ref={panelRef} className={props.className}>
			{props.children}
		</div>
	);
});
...
const useEasyResizable = (opts: UseResizableProps) => {
	const { isDragging, position, separatorProps, ...rest } = useResizable({
		...opts,
		onResizeStart: (...args) => {
	...
	return {
		Separator: () => (
			<div
				...
				tabIndex={0}
				className={cn(
					"w-1 min-w-1 cursor-col-resize bg-charcoal-500 transition-colors hover:bg-gray-600",
