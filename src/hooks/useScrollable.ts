...
const useScrollable = <T extends BaseType>({
	...
	const atBottom = useCallback(
		(offset = 0) => {
			return ref.current
				? Math.floor(ref.current?.scrollHeight - ref.current?.scrollTop) - offset <= ref.current?.clientHeight
				: true;
		},
		[ref],
	);
				...
				if (scrollState.current.type === ScrollStates.ScrollToBottom) {
					...
					animateScroll.scrollTo(
						Math.max(
							101,
							ref.current
								? ref.current.scrollTop + (ref.current.scrollHeight - scrollState.current.previousHeight)
								: 101,
						),
						{
							container: ref.current,
	...
	const fetchTop = useCallback(
		async (count: number, noAround = false, force = false) => {
			if (!ref.current) {
				return;
			}
			...
			for (const child of Array.from(ref.current.children).reverse()) {
				// ? we reverse the array since this is the top and we want to remove the bottoms
				if (idsToRemove.length >= removeCount || child.id.startsWith("toIgnore")) {
					// ? if we are above we can just continue, likely could just slice the array but this is fine thats marginal in performance
					continue;
				}
			...
			setRenderedItems((prev) =>
				[...newItems.items, ...prev].filter((item) => !idsToRemove.includes(item.id)).sort(sortItems),
			);
	...
	const fetchBottom = useCallback(
		async (count: number, noAround = false, force = false) => {
			...
			setRenderedItems((prev) =>
				[...prev, ...newItems.items].filter((item) => !idsToRemove.includes(item.id)).sort(sortItems),
			);
			setHasMoreTop(true);
