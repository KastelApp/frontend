...
const Reset = () => {
    ...
    const validateToken = async () => {
        const [validated, error] = await safePromise(client.api.post<{
            id: string;
            token: string;
        }>({
            url: "/auth/reset",
    ...
    const onReset = async () => {
        ...
        const [reset, error] = await safePromise(client.api.patch<{
            id: string;
            token: string;
            password: string;
        }, {
            ...
            <HomeLayout>
                <div className="flex justify-center items-center">
                    <Card className="flex items-center justify-center mt-32 w-full max-w-md p-8 bg-lightAccent dark:bg-darkAccent">
                        {ready ? <div className="w-full max-w-md">
                            <div className="text-center">
                                <h1 className="text-3xl font-bold">{t("reset.title")}</h1>
                                <p className="text-medium mt-4">{t("reset.subtitle")}</p>
                            </div>
                            ...
                            <form className="mt-4">
                                <div className="flex flex-col space-y-4 items-center">
                                    <Input
                                        ...
                                        endContent={
                                            <button className="focus:outline-none" type="button" onClick={() => setPasswordVisible((p) => !p)} tabIndex={-1}>
                                                {passwordVisible ? (
                                    ...
                                    <Input
                                        ...
                                        endContent={
                                            <button className="focus:outline-none" type="button" onClick={() => setConfirmPasswordVisible((p) => !p)} tabIndex={-1}>
                                ...
                                <div className="mt-8">
                                    <Button
                                        ...
                                        tabIndex={3}
                                    >
                                        {loading ? <LoaderCircle className="text-white custom-animate-spin" /> : t("reset.button")}
                                    </Button>
                                </div>
                                <div className="mt-4 flex justify-between">
									<Link href="/login" color="primary" className="text-sm" as={NextLink} tabIndex={4} onClick={async () => {
                                        const [,err] = await safePromise(client.api.del<{
                                            id: string;
                                            token: string;
                                        }>({
                                            url: "/auth/reset",
                        ...
                        </div> : <LoaderCircle size={72} className="text-primary custom-animate-spin" />}
                    </Card>
                </div>
