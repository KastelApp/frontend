...
const RoleMention = ({
  ...
  const role = roles.find((role) =>
    roleId === "@everyone" ? role.id === role.guildId : role.id === roleId,
  );

  return (
    <Mention
      onClick={onClick}
      mentionBg={role ? hexToHsl(role.hexColor) : ""}
      hoverBg={role ? hexToHsl(role.hexColor, 0.5) : ""}
      noUnderline
    >
      <Text as="span" color={role ? role.hexColor : ""}>
        {["@everyone", "@here"].includes(roleId)
          ? roleId
