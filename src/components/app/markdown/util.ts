...
export const recurse = (node: SimpleMarkdown.RefNode, recurseOutput: (content: unknown, state: unknown) => React.ReactElement, state: { key: string; }) => {
    return typeof node.content === "string"
        ? node.content
        : recurseOutput(node.content, state);
}

export const recurseStrings = (node: SimpleMarkdown.RefNode, recurseOutput: (content: unknown, state: unknown) => React.ReactElement, state: { key: string; }): string => {
    if (Array.isArray(node.content)) {
        return node.content.map((content) => recurseStrings(content, recurseOutput, state)).join("");
    } else if (typeof node.content === "string") {
        return node.content;
    } else {
