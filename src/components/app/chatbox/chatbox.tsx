...
} from "../markdown/defaultEmojis.ts";
    ...
    <AutoResizeTextarea
      ...
      onSelect={(event: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        for (let i = 0; i < splitWords.length; i++) {
          ...
          if (
            ...
            setMentioning({
              isMenioning: true,
              searchingFor: beforeCaret.substring(
                beforeCaret.lastIndexOf("@") + 1,
              ),
              members: mappedMembers.filter((member) =>
                member.user.username
                  .toLowerCase()
                  .includes(
                    beforeCaret
                      .substring(beforeCaret.lastIndexOf("@") + 1)
                      .toLowerCase(),
                  ),
              ),
            ...
            const emojis = getEmojisThatMatchKeyword(
              beforeCaret.substring(beforeCaret.lastIndexOf(":") + 1),
              1,
              10,
            );
      ...
      onInput={(event: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        if (event.currentTarget.value.length > 0) {
          if (
            Date.now() - typing.lastTypedSent > 10000 &&
            typing.startedTyping > 0 &&
            Date.now() - typing.startedTyping > 3000
          ) {
            setTyping({
