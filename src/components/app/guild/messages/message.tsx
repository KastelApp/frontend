...
import {
  ModelData,
  clickHandler,
} from "../../markdown/components/mentions/mentionData.tsx";
        ...
        >
          ...
          {replyMessage ? (
            <Avatar
              ...
              width={4}
              ml={2}
              src={replyMessage.user?.getAvatarUrl({ size: 128 })}
            />
          ) : (
            <Box
              height={4}
          ...
          {replyMessage && (
            <Text ml={2} fontSize="xs" color={topRole?.hexColor ?? ""}>
              {replyMessage.member?.displayUsername}
            </Text>
          )}
          <Text
            fontSize="xs"
            ml={2}
            color={getMessageColor(replyMessage?.message?.state ?? "sent")}
            maxW={"calc(100vw - 500px)"}
            overflow={"hidden"}
        ...
        <Box ml="3" position={"relative"}>
          {!chunked && (
            <Text cursor={"default"}>
              ...
              <Tooltip
                label={message.creationDate.toLocaleString("en-US", {
                  weekday: "long",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
