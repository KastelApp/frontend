...
import { useChannelStore, useChannelStoreWithNoMessages, useGuildStore, useMemberStore, useMessageStore, useRoleStore, useUserStore } from "$/utils/Stores.ts";
...
import { useClientStore, useReadyStore, useTokenStore } from "@/utils/stores.ts";
...
const TextChannel = () => {
    ...
    const { channels: noMoreChannels, addChannel } = useChannelStoreWithNoMessages();
    ...
    const populateMessages = async (override = false) => {
        ...
        const newMessages = await fetchMessages(50, !override ? lastMessage.id : undefined);
    ...
    return (
        <>
            <Box maxHeight="calc(100vh - 100px)" ml={-2} mr={-2} id="scrollable-div">
                <GuildMessages ready={areWeReady} chunks={messages} modelData={model} hasMore={hasMore} setData={setData} next={populateMessages} fullMessages={messageStore.messages} />
                <div id="bottom-chat" />
            </Box>
            <Popover isOpen={isOpen} onClose={onClose} placement={placement} isLazy>
                <PopoverTrigger>
                    <Box position="absolute" top={y + height} left={placement === "right" ? x + width : x} />
                </PopoverTrigger>
                <Portal>
                    {data && data.type === "user" ? (
