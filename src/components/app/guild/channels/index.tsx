...
const RawChannel = ({ channel }: { channel: BaseChannel }) => {
  const collapsedChannelsList = useCollapsedChannels(
    (s) => s.collapsedChannels,
  );
  ...
  return (
    <>
      <Flex ml={channel.parentId ? 1 : 0}>
        ...
        {channel.isCategory() && (
          <Flex cursor={"pointer"}>
            <Text color={textColor}>{getChannelName(channel?.name)}</Text>
            <svg
              width="16px"
              height="16px"
              ...
              style={{
                transform: collapsedChannelsList.includes(channel.id)
                  ? "rotate(270deg)"
                  : "rotate(360deg)",
                marginTop: "4px",
...
const Channel = ({
  ...
  return (
    <Flex
      ...
      ml={5}
      key={channel?.id}
      bg={channel?.id === currentChannel?.id ? bg : ""}
      rounded="10px"
      userSelect={"none"}
      cursor={"pointer"}
      _hover={
