...
import {
	...
	useMessageStore,
} from "@/wrapper/Stores/MessageStore.ts";
...
const TextBasedChannel = () => {
	...
	const messages = useMessageStore((state) =>
		state.getMessages(channelId).sort((a, b) => a.creationDate.getTime() - b.creationDate.getTime()),
	);
	...
	const fetchBottomItems = useCallback(
		async (count: number, around?: string | null) => {
	...
	const fetchTopItems = useCallback(
		async (count: number, around?: string | null) => {
			const aroundIndex = messages.findIndex((msg) => msg.id === around);
			const cachedMessages = messages.slice(Math.max(0, aroundIndex - count), aroundIndex);
	...
	const messageSubscription = useCallback(
		(state: MessageStore, prevState: MessageStore) => {
	...
	const slowedTopFetch = useCallback(
	...
	const slowedBottomFetch = useCallback(
		async (count: number, noAround = false, force = false) => {
	...
	const createRenderableMessages = useCallback(
