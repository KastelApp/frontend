...
import { astToString } from "@/utils/markdown.ts";
    ...
    return {
        ...
        blockQuote: {
            ...
            react: (node: SimpleMarkdown.RefNode, recurseOutput: (content: unknown, state: unknown) => React.ReactElement, state: { key: string; }) => (
                <blockquote key={state.key} className="border-l-4 border-gray-400 pl-2">{recurseOutput(node.content, state)}</blockquote>
            )
        },
        list: {
            ...defaultRules.list,
        ...
        inlineCode: {
            ...inlineCode,
            react: (node: SimpleMarkdown.RefNode, _: (content: unknown, state: unknown) => React.ReactElement, state: { key: string; }) => (
                <Code key={state.key} className="text-gray-300">{node.content as unknown as string}</Code>
            )
        },
        paragraph: {
            ...paragraph,
