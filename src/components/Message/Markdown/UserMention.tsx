...
const UserMention = ({
    ...
    return (
        <PopOverData user={foundUser!} onlyChildren={!foundUser} member={foundMember ? {
            ...foundMember,
            roles: foundMember.roles.map((roleId) => roles.find((role) => role.id === roleId)!)
        } : null}>
            <span className={cn("rounded-lg p-1 font-medium hover:underline cursor-pointer", !topRole && "bg-branding-300/25 text-branding-100")} style={{
