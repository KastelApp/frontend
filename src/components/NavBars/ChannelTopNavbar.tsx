		...
		<TopNavBar>
			<div className="flex items-center">
				<Button
					...
					className={isMobile && isChannelsOpen ? "ml-auto" : ""}
				>
					<Sidebar className="h-5 w-5 text-gray-400" />
				</Button>
				{((isMobile && !isChannelsOpen) || !isMobile) && (
					<>
						<Hash className="ml-2 h-5 w-5 text-gray-400" />
						<span className="ml-2 truncate font-semibold text-white">{channel.name || "Unknown Channel"}</span>
						{channel.description && (
							<>
								<Divider orientation="vertical" className="ml-2 mr-2 h-6 w-[3px]" />
								<span
									className="flex w-96 cursor-pointer items-center truncate whitespace-pre-wrap break-words text-sm text-gray-400"
									onClick={() => {
										modalStore.getState().createModal({
											...
											body: (
												<div className="flex items-center whitespace-pre-wrap break-words">
													<MessageMarkDown>{channel.description!}</MessageMarkDown>
												</div>
											),
										});
									}}
