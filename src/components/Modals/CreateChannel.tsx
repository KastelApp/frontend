...
const CreateChannelModal = ({ isOpen, onOpenChange }: { isOpen: boolean; onOpenChange: () => void; onClose: () => void; }) => {
        ...
        >
            <ModalContent className="overflow-hidden">
                ...
                <ModalBody>
                    ...
                    {tab === 0 ? (
                        <RadioGroup className="overflow-y-scroll max-h-96 max-w-[28rem] p-2" value={selectedChannel}>
                            {Object.entries(mappings).filter(([k]) => ![Constants.channelTypes.Dm.toString(), Constants.channelTypes.GroupChat.toString()].includes(k)).map(([key, value]) => (
                                <CustomRadio
                                    key={key}
                                    value={key}
                                    onChange={() => {
