...
<script lang="ts">
	...
	export let canBeTogged = false;
	export let id: string = `${title}-${name}`;
	...
	const writeableOption = writable({
		name,
		enabled,
		canBeTogged
	});
    ...
    writeableOption.subscribe(async (value) => {
        ...
        const foundCheckbox = foundModal.checkboxeOptions.find((checkbox) => checkbox.id === id);

        if (!foundCheckbox) {
            console.warn('No Checkbox found');
            
            return;
        }

        foundCheckbox.enabled = value.enabled;
    });
</script>
	...
	<div class="">
		...
		<div class="top-16 relative gap-[44px] unselectable checkboxItems">
			...
			<div
				...
				on:click={() => {
					$writeableOption.enabled = !$writeableOption.enabled;
				}}
			>
				{#if $writeableOption.enabled}
					<svg
						class="relative"
						width={30}
						height={29}
					...
					>
						...
						<path
							...
							fill="white"
						/>
					</svg>{:else if !$writeableOption.enabled}
					<svg
						class="relative"
						width={30}
						height={29}
